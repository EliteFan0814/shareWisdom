  ...
  >
    <el-form :model="data" label-position="right" ref="data" :rules="rules" label-width="15%">
      <el-form-item label="栏目：" prop="classid">
        <el-cascader class="inp" v-model="data.classid" :options="list" :props="prop_good" @change="getData" placeholder="请选择栏目" :show-all-levels="true" clearable>
          </el-cascader>
      </el-form-item>
      ...
      <el-form-item label="缩略图：">
        <el-upload
          ...
          >
            <div class="src" v-if="data.picurl">
      ...
      <el-form-item label="详细内容：">
        <Editor class="inp editor" id="tinymce" :content="data.content" @changed="(value)=>{this.data.content=value}"></Editor>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      params.append("classid", this.data.classid[this.data.classid.length-1]);
      ...
      this.$http.post("/manage/info_list/add", params).then(res => {
        if (res.code == 1) {
          this.$emit('close','1')
        }
      });

  ...
  data() {
    return {
      ...
      rules: {
        ...
        // picture_json:[{ required: true, message: '请上传图纸' }]
      }
    }
  },
  methods: {
    ...
    // 上传前触发钩子
    beforeUpload(file) {
      console.log('上传前触发钩子file:', file)
      this.before_up_pic.push(file)
    },
    // 上传图片
    uploadPic(upfiles) {
      ...
      this.$http.post('/company/image/upload', form).then(res => {
        if (res.code) {
          ...
          console.log('res：', res)
          now_pic.url = res.data.fileurl
          this.pic_info_list.push(now_pic)
          console.log('图片列表详情：', this.pic_info_list)
        }
      })
    },
    // 删除图片
    removeImg(file, fileList) {
      ...
      console.log('删除后的图片列表信息：', fileList)
      this.before_up_pic = []
      for(let i = 0;i<fileList.length;i++){
        this.before_up_pic.push(fileList[i].raw)
      }
      console.log('this.pic_list',this.before_up_pic)
    },
    submit(refName) {

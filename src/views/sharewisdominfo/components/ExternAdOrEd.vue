<template>
  <el-dialog :title="rowInfo.order_id?'编辑当前职位':'发布新职位'" :visible.sync="isDialog" :close-on-click-modal="false" @close="close" :center="true" width="50%">
    <el-form :rules="rules" :model="innerRowInfo" ref="rowInfo" label-position="right" label-width="12%">
      ...
      <el-form-item label="行业：" prop="trade_id">
        <el-select v-model="innerRowInfo.trade_id" placeholder="请选择行业">
          <el-option v-for="item in trade_class" :key="item.trade_id" :label="item.name" :value="item.trade_id"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="添加图纸：" prop="picture_json">
        <el-upload class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/" :http-request="uploadPic" :on-remove="removeImg" :file-list="el_img_list" list-type="picture-card">
          <el-button size="small" type="primary">点击上传</el-button>
        </el-upload>
      </el-form-item>

      <el-form-item label="公司地址：" prop="map_location">
        <Amap @getPosition="readPosition" :th_position="rowInfo" :user_id="rowInfo.order_id" :what_class="'order'"></Amap>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    uploadPic(upfiles) {
      ...
      this.$http
        .post('/company/image/upload', form)
        .then(res => {
          if (res.code) {

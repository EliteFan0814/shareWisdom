      ...
      <div class="body">
        <div v-for="item in tableData" class="block">
          <div class="itemC">
            ...
            <el-input style="float: left;" v-if="item.code === 'customer_service'" v-model="item.value" placeholder="请输入电话"></el-input>
            <el-slider v-else v-model="item.value" class="slid" show-input></el-slider>
          </div>
        </div>
    ...
    getData() {
      this.$http.get('/api/SystemSetting/List').then(res => {
        ...
        for(let i = 0;i < systemStr.length;i++){
          if(!systemStr[i].code.indexOf('customer_service') || !systemStr[i].code.indexOf('out_commission')){
